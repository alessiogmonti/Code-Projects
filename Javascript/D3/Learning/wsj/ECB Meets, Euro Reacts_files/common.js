	...
	*/
    ...
    function set_moment_defaults(){
		...
		function create_menu(){
			$.getJSON('https://www.wsj.com/fetch/nav',function(data){
				// Show menu icon on data response
				$('#top-menu').show();
				// Create menu
				var menu = '';
				$.each(data['na,us'].items,function(i,d){
					var url = d.url.match('wsj.com') !== null ? d.url : '//www.wsj.com' + d.url;
					menu += '<div class="top-menu-item"><a href="' + url + '">' + d.label + '</a></div>';
				});
				$('#top-menu-container').append(menu);
			});
		}
    function top_menu(){
      $('#top-menu').click(function(){
        $('#top-menu-container,#top-menu-screen,.top-header-menu,#top-close').show();
				$('#top-menu').hide();
				// $('.top-header,.pro-header').hide();
				$('#top-menu-screen').css({
					width:window.innerWidth,
					height:window.innerHeight
				});
      });
			$('#top-close,#top-menu-screen').click(function(){
				$('#top-menu-container,#top-menu-screen,.top-header-menu,#top-close').hide();
				$('#top-menu').show();
				// $('.top-header,.pro-header').show();
			});
		...
		function top_share_menu(){
			$('#top-share-button').click(function(){
        $('#top-share-container').show();
				$('#top-share-screen').css({
					width:window.innerWidth,
					height:window.innerHeight
				}).show();
				$('#top-menu-container,#top-menu-screen,.top-header-menu,#top-close').hide();
			...
			$('#top-share-close,#top-share-screen').click(function(){
				$('#top-share-container,#top-share-screen').hide();
			});
			$(window).resize(function(){
				$('#top-share-screen').css({
					width:window.innerWidth,
				...
				// If larger than mobile view, hide the share menu
			...
			$('#top-share-container .top-share-cancel').click(function(){
				$('#top-share-container,#top-share-screen').hide();
				return false;
			});
		}

		function responsive_ad_code(){
			var view = window.innerWidth <= 768 ? 'mobile' : window.innerWidth <= 991 ? 'tablet' : 'desktop';
			$(window).resize(function(){
				var newView = window.innerWidth <= 768 ? 'mobile' : window.innerWidth <= 991 ? 'tablet' : 'desktop';
				if(newView != view){
