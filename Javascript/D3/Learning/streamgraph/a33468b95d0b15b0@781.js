// https://observablehq.com/@d3/color-legend@781
...
function legend({color, ...options}) {
  ...
  main.variable(observer("Swatches")).define("Swatches", ["htl"], function(htl){return(
...
.${id}::before {
  ...
  <div>${color.domain().map(value => htl.html`<span class="${id}" style="--color: ${color(value)}">${format(value)}</span>`)}</div>`;
}
)});
